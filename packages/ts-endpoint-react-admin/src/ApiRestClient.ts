...
export const paramsToPagination = (start: number, end: number): RA.GetListParams['pagination'] => {
  return {
...
export const APIRESTClient = ({ getAuth, ...ctx }: APIRESTClientCtx): APIRESTClient => {
  ...
  return {
    ...
    get: (url, params) => liftClientRequest(() => client.get(url, { params }))(),
    ...
    create: (resource, params) => {
      return liftClientRequest<RA.CreateResult>(() => client.post(`${resource}`, params.data))();
    },
    ...
    delete: (resource, params) => client.delete(`${resource}/${params.id}`, { params }),
